...
#+begin_src emacs-lisp
  (use-package notmuch
    ...
    <<notmuch-new-async>>
    <<notmuch-tree-scroll-back-or-previous>>
    <<notmuch-search-show-thread-maybe-tree>>
    :custom
    ...
    (:map notmuch-search-mode-map
          ("RET" . notmuch-search-show-thread-maybe-tree))
    (:map notmuch-message-mode-map
          ("C-x C-s" . ignore))
    (:map notmuch-tree-mode-map
          ("DEL" . notmuch-tree-scroll-back-or-previous))
...
A bunch of notmuch-related snippets.  Functions should go in =:init=.
...
#+begin_src emacs-lisp :tangle no
  ...
  PROMPT and COLLECTION are both passed to `completing-read'."
    (unless collection
      (pcase (car notmuch-address-internal-completion)
        ('sent (setcar notmuch-address-internal-completion 'received))
        ('received (setcar notmuch-address-internal-completion 'sent)))
      (setq notmuch-address-last-harvest 0
            notmuch-address-completions (clrhash notmuch-address-completions)
            notmuch-address-full-harvest-finished nil))
    (completing-read
...
***** Scroll back or previous message

In a tree search, =SPC= is bound to =notmuch-tree-scroll-or-next=.  I
very much like the way this works, but =DEL= is just bound to a dumb
scroll.  This is a similar function, in reverse.

#+name: notmuch-tree-scroll-back-or-previous
#+begin_src emacs-lisp :tangle no
  (defun notmuch-tree-scroll-back-or-previous ()
    "Scroll the message window back.  If it's already at the
  beginning, go to previous message."
    (interactive)
    (when (notmuch-tree-scroll-message-window-back)
      (notmuch-tree-prev-matching-message)))
#+end_src

***** Show thread in tree when thread is large

When reading large threads (i.e. from mailing lists),
=notmuch-show-mode= gets really slow.  A simple solution is to show
large threads in =notmuch-tree-mode=.

#+name: notmuch-search-show-thread-maybe-tree
#+begin_src emacs-lisp :tangle no
  (defun notmuch-search-show-thread-maybe-tree (&optional choice)
    "Display the currently selected thread.

  If the thread length is greater than 40 or if prefix arg CHOICE
  is positive, show it in `notmuch-tree-mode'.

  If the thread length is less than 40 or if prefix arg CHOICE is
  negative, show it in `notmuch-show-mode'."
    (interactive "p")
    (let* ((results (notmuch-search-get-result))
           (thread-id (concat "thread:" (plist-get results :thread)))
           (subject (plist-get results :subject))
           (buffer (format "*%s*" (truncate-string-to-width subject 30 nil nil t)))
           (length (plist-get results :total))
           (use-tree (cond
                      ((< choice 0) nil)
                      ((> choice 1) t)
                      ((> length 40) t))))
      (when (null thread-id)
        (user-error "End of search results."))
      (if use-tree
          ;; Use `notmuch-tree-mode'
          (notmuch-tree thread-id
                        notmuch-search-query-string
                        nil buffer t)
        ;; Use `notmuch-show-mode'
...
Notmuch provides a facility to quickly tag messages or threads.
=notmuch-tag-jump= is bound to =k=, and can be customised with the
variable =notmuch-tagging-keys=.  Similar to =notmuch-jump=.

#+name: notmuch-tagging-keys
#+begin_src emacs-lisp :tangle no
  '(("a" ("+archive")
     "Archive")
    ("u" ("-unread")
