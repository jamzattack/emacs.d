...
A way to open URLs the way I want.  I bind it to =C-z d=.  I use the
=:demand= keyword so that I can use these commands in the [[*EWW][EWW]] and
[[*Elfeed][Elfeed]] use-package forms.
...
#+begin_src emacs-lisp
  (use-package custom-exwm-config
    ...
    :hook
    (server-after-make-frame-hook . custom-exwm-config))
#+end_src
...
#+begin_src emacs-lisp
  (use-package helpful
    ...
    (helpful :type git
             :flavor melpa
             :host nil
             :repo "git@gitlab.com:jamzattack/helpful"
             :branch "no-f")
    :config
...
#+begin_src emacs-lisp
  (use-package selime
    ...
    (selime :type git
            :flavor melpa
            :repo "git@gitlab.com:jamzattack/selime")
...
# eval: (rename-buffer "config.org" t)
# End:
