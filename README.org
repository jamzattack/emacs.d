...
#+begin_src emacs-lisp
  (use-package dmenu-handler
    ...
    :init (autoload 'eww-mode-map "eww" nil nil 'keymap)
    :bind
    (("C-z d" . 'dmenu-handler)
...
* Packages installed with Straight
...
I sent in a [[file:lisp/helpful/helpful-no-f.diff][patch]] to remove helpful's dependency on f.el, so I use a
local version rather than the one in melpa.

#+begin_src emacs-lisp
  (use-package helpful
    :load-path "lisp/helpful"
    :init
